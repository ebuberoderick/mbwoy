...
const ResponseModal = ({ isOpen, onClose, message,status }) => {
  ...
  return (
    <div className="fixed inset-0 p-3 z-50 flex items-center justify-center backdrop-blur-md bg-white bg-opacity-5">
      <div className={`relative bg-white rounded-lg space-y-4 px-4 py-4 shadow-lg overflow-hidden w-full max-w-sm mx-auto`}>
        {
          status ? <Image src={success}  alt="#" draggable={false} className="pointer-events-none w-20 mx-auto" />:<Image src={error}  alt="#" draggable={false} className="pointer-events-none w-20 mx-auto" />
        }
